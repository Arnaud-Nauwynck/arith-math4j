...
public class ContinuedFraction {
	...
	public BigIntegerFraction getConvergent(int n) {
		if (e_frac == null)
			throw new IllegalArgumentException("Fraction period needed");
		return new BigIntegerFraction(h(n), k(n));
	}
	 ...
	 */
	private static ArrayList<Integer> getPeriod(int S) {
		...
		if (fraction.size() > 1 && 2 * a0 != result.get(result.size() - 1))
				throw new RuntimeException("no pattern found for "+ S);
		return result;
	}
	...
	public static ContinuedFraction fromSquare(int S)  {
